...
temperature_readings = [22, 35, 28, 40, 18, 30]
threshold = 30
for i, temp in enumerate(temperature_readings):
    if temp > threshold:
        print(f"Warning: Temperature {temp}°C at index {i} exceeds the threshold of {threshold}°C.")
...
for product, details in products.items():
    ...
    if stock < min_required:
        print(f"Restock Alert: {product.capitalize()} stock is {stock}, below the minimum required {min_required}.")
