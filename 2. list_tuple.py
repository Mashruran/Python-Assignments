...
for person_id, details in people.items():
    if len(details) == 3:  
        name, age, is_student = details
        print(f"{person_id}: Name: {name}, Age: {age}, Student: {is_student}")
    else:
        print(f"{person_id} contains unexpected tuple structure: {details}")
...
students = [("Alice", 85), ("Bob", 75), ("Charlie", 95), ("Diana", 90)]

# Sort the list by grades (ascending order)
students_sorted = sorted(students, key=lambda x: x[1])

print("Students sorted by grades:")
for student, grade in students_sorted:
